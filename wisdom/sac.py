...
class PolicyTrainer:
    def __init__(
        ...
        if self.use_automatic_entropy_tuning:
            if target_entropy:
                ...
                self.target_entropy = -self.env_action_space
    ...
    def training_step(self, indices, step):
        ...
        in_z = task_z.view(task_z.shape[0], -1, task_z.shape[-1])
        pred_z, _ = self.z_model(in_z)
        new_task_z = pred_z.view(-1, in_z.shape[-1])
        # train the SAC and the state is just extended to be [state , z]
        obs = torch.cat((obs, new_task_z.detach()), dim=1)
        next_obs = torch.cat((next_obs, new_task_z.detach()), dim=1)
        ...
        if self._need_to_update_eval_statistics:
            ...
            self.eval_statistics['QF Loss'] = np.mean(ptu.get_numpy(qf1_loss))
