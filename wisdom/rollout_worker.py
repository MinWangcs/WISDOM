...
class RolloutCoordinator:
    ...
    def collect_data(self, tasks, train_test, deterministic=False, max_samples=np.inf, max_trajs=np.inf, animated=False, save_frames=False):
        ...
        if self.use_multiprocessing:
            ...
            workers = [RemoteRolloutWorker.remote(None, self.env_name, self.env_args, self.train_or_showcase,
                                                  ...
                                                  task_list, self.tasks, self.train_tasks, self.test_tasks) for task_list in tasks_per_worker]
    ...
    def evaluate(self, train_test, tasks, num_eval_trajectories, deterministic=True, animated=False, save_frames=False, log=True):
        ...
        if log:
            ...
            eval_statistics[train_test + '_eval_avg_reward'] = eval_average_reward
            eval_statistics[train_test + '_eval_std_reward'] = eval_std_reward
            eval_statistics[train_test + '_eval_max_reward'] = eval_max_reward
            eval_statistics[train_test + '_eval_min_reward'] = eval_min_reward
...
class RolloutWorker:
    ...
    def obtain_samples(self, task, train_test, deterministic=False, max_samples=np.inf, max_trajs=np.inf, animated=False, save_frames=False):
        ...
        while n_steps_total < max_samples and n_trajs < max_trajs:
            if hasattr(self.env,'tasks_pool'):
                self.env.set_task(self.env.tasks_pool[task])
            else:
    ...
    def rollout(self, deterministic=False, max_path_length=np.inf, animated=False, save_frames=False):
        ...
        while path_length < max_path_length:
            ...
            out = self.agent.get_action(agent_input, o, deterministic=deterministic)
            a, agent_info = out[0]
        ...
        next_task_indicator = self.agent.get_action(agent_input, next_o, deterministic=deterministic)[1]
